...
public class lesson4 {
    ...
    private static boolean chekLine(char symb) {
        int cnt = 0;
        for (int i = CurY; (i < SIZE) && (cnt <= DOTS_TO_WIN); i++) { //подсчет от точки вправо
            if (map[CurX][i] == symb) {
                cnt = cnt + 1;
                if (cnt == DOTS_TO_WIN) {
                    return true;
        ...
        for (int i = CurY - 1; (i >= 0) && (cnt <= DOTS_TO_WIN); i--) { //подсчет от точки влево
            if (map[CurX][i] == symb) {
                cnt = cnt + 1;
                if (cnt == DOTS_TO_WIN) {
                    return true;
    ...
    private static boolean chekRow(char symb) {
        int cnt = 0;
        for (int i = CurX; (i < SIZE); i++) { //подсчет от точки вверх
            if (map[i][CurY] == symb) {
                cnt = cnt + 1;
                if (cnt == DOTS_TO_WIN) {
                    return true;
        ...
        for (int i = CurX - 1; (i >= 0); i--) { //подсчет от точки вниз
            if (map[i][CurY] == symb) {
                cnt = cnt + 1;
                if (cnt == DOTS_TO_WIN) {
                    return true;
    ...
    public static void aiTurn() {
        ...
        System.out.println("Комп сходил в точку " + (CurX + 1) + " " + (CurY + 1));
        map[CurX][CurY] = DOT_O;
    }
